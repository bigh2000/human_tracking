...
def pose_nms(bboxes, bbox_scores, pose_preds, pose_scores):
    ...
    while(human_scores.shape[0] != 0):
        ...
        bbox.append(bboxes[human_ids[delete_ids[0]]]) ##
...
def write_json(all_results, outputpath, for_eval=False):
    ...
    for im_res in all_results:
        ...
        for human in im_res['result']:
            ...
            kal_idx = []
            if for_eval:
                #result['frame'] = int(im_name.split('/')[-1].split('.')[0].split('_')[-1])
                result['frame'] = im_name
            else:
                #result['frame'] = im_name.split('/')[-1]
                result['frame'] = im_name

            kp_preds = human['keypoints'].numpy().tolist()
            person_id = human['person_id']
            for n in range(len(kp_preds)):
                kp_scores.append(float(human['kp_score'][n]))

            result['kp_scores'] = kp_scores
            result['kp_coordinates'] = kp_preds
            result['score'] = float(human['proposal_score'])
            result['person_id'] = person_id

            result['bbox'] = human['bbox'].numpy().tolist()

            kal_idx.append(person_id)
            kal_idx.append(im_name)
            kal_idx.extend([1,1,0,0,0,0])
            kal_idx.extend(human['bbox'].numpy().tolist())
            ...
            if form == 'cmu': # the form of CMU-Pose/OpenPose
                ...
                json_frame.append(result)
                kal_dets.append(kal_idx)
        json_results_frame[im_name] = json_frame
    json_results = json_results_frame
